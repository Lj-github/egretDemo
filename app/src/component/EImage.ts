...
class EImage extends egret.Bitmap
{
    ...
    private onLoadBack(img:HTMLImageElement, url:string):void{
        ...
        if(url==this._src) {
            if (img) {
                ...
                // let imgReview: egret.Bitmap = new egret.Bitmap(tt);
                tt._setBitmapData(bitmapdata);
                this.texture = tt;
            } else {
                this.texture = null;
            }
    ...
    public load(url:string):void
    {
        ...
        if(url!=null && url.length>0){
            ...
            img["$"] = this;
            img.onload = function(){
                EImage.$(this).onLoadBack(<HTMLImageElement>this, (<HTMLImageElement>this).src);
            };
